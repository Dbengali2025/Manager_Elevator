...
import { getBillingAccess, getBillingUser, findBillingCustomer, TRIAL_DAYS } from "@/lib/billing";
...
export default async function BillingPage({ searchParams }: { searchParams: { session_id?: string; canceled?: string; plan?: string } }) {
  ...
  return <main className="min-h-screen bg-offWhite px-md py-xl sm:px-lg">
    <div className="mx-auto max-w-3xl space-y-lg">
      ...
      <h1 className="font-heading text-h1 text-navy">{access.allowed ? (access.trial ? "Your free trial" : "Your membership") : "Choose your membership"}</h1>
      ...
      {access.allowed && <div className="rounded-lg bg-mintGreen/30 p-lg">
        <p className="mb-md text-body text-navy">{user.isAdmin ? "Administrator access" : access.complimentary ? "You have complimentary access." : access.trial ? `You're on your free ${TRIAL_DAYS}-day trial — it ends ${access.trialEndsAt ? new Date(access.trialEndsAt).toLocaleDateString("en-US", { month: "long", day: "numeric" }) : "soon"}. Choose a plan below to keep your access (and unlock all module downloads).` : "Your membership is active."}</p>
      ...
      <BillingControls hasCustomer={!!customer} subscribed={!!subscription || (access.allowed && !access.trial)} selectedPlan={isBillingPlan(searchParams.plan) ? searchParams.plan : undefined} />
    </div>
  </main>;
}
